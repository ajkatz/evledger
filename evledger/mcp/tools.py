...
``$EVLEDGER_ROOT`` → the per-user default ``~/.local/share/evledger/ledger``
...
def resolve_ledger_root(
    ...
    2. The ``$EVLEDGER_ROOT`` environment variable, if set and non-blank.
...
def ledger_log(
    ...
    *,
    ...
    Args:
        ...
        root: Explicit ledger root; else ``$EVLEDGER_ROOT``; else the
